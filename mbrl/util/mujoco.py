...
class FreezeCustom(Freeze):
    """Provides a context to freeze a custom environment.
    This context allows the user to manipulate the state of a Mujoco environment and return it
    to its original state upon exiting the context.
    Example usage:
    .. code-block:: python
    ...
    def __enter__(self):
        self._init_state =  self._env.env.state
        self._elapsed_steps = self._env._elapsed_steps
...
class CustomEnvHandler(EnvHandler):
    """Env handler for my custom envs"""
